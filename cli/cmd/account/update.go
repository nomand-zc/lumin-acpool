...
func (c *updateCmd) run(cmd *cobra.Command) error {
	...
	oldAcct, err := deps.AccountStorage.Get(cmd.Context(), raw.ID)
	if err != nil {
	...
	if providerType != oldAcct.ProviderType || providerName != oldAcct.ProviderName {
		...
		if _, err := deps.ProviderStorage.Get(cmd.Context(), providerKey); err != nil {
			if err == storage.ErrNotFound {
	...
	if err := deps.AccountStorage.Update(cmd.Context(), updated); err != nil {
