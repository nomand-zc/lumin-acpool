...
func (c *removeCmd) runSingleRemove(cmd *cobra.Command) error {
	...
	// 删除账号
	if err := deps.AccountStorage.Remove(cmd.Context(), c.accountID); err != nil {
		return handleStorageError("Account", err)
	}

	// 清理关联的统计数据
	if err := deps.StatsStore.Remove(cmd.Context(), c.accountID); err != nil {
		// 统计数据清理失败不阻塞主流程，仅打印警告
		fmt.Printf("警告: 清理统计数据失败: %v\n", err)
	}

	// 清理关联的用量追踪数据
	if err := deps.UsageStore.Remove(cmd.Context(), c.accountID); err != nil {
...
func (c *removeCmd) runBatchRemove(cmd *cobra.Command) error {
	...
	// 批量删除账号
	if err := deps.AccountStorage.RemoveFilter(cmd.Context(), filter); err != nil {
	...
	for _, a := range accounts {
		if err := deps.StatsStore.Remove(cmd.Context(), a.ID); err != nil {
			cleanupErrs++
		}
		if err := deps.UsageStore.Remove(cmd.Context(), a.ID); err != nil {
			cleanupErrs++
		}
	}
