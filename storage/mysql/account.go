...
func (s *Store) UpdateAccount(ctx context.Context, acct *account.Account, fields storage.UpdateField) error {
	...
	if fields.Has(storage.UpdateFieldStatus) {
		...
		args = append(args, int(acct.Status), acct.CooldownUntil, acct.CircuitOpenUntil)
	}
	...
	if len(setClauses) == 0 {
		return fmt.Errorf("mysql store: no fields to update")
	}
	...
	if fields.Has(storage.UpdateFieldStatus) {
		return s.client.Transaction(ctx, func(tx *sql.Tx) error {
			...
			if txErr := row.Scan(&oldStatusInt); txErr != nil {
				...
				return fmt.Errorf("mysql store: failed to get old status: %w", txErr)
			}

			// 执行更新
			result, txErr := tx.ExecContext(ctx, query, args...)
			if txErr != nil {
				return fmt.Errorf("mysql store: failed to update account: %w", txErr)
			}
			rowsAffected, txErr := result.RowsAffected()
			if txErr != nil {
				return fmt.Errorf("mysql store: failed to get rows affected: %w", txErr)
			}
			if rowsAffected == 0 {
			...
			if oldStatus != newStatus {
				...
				if delta != 0 {
					_, txErr = tx.ExecContext(ctx,
						`UPDATE providers SET available_account_count = GREATEST(available_account_count + ?, 0), updated_at = NOW(3) WHERE provider_type = ? AND provider_name = ?`,
						delta, acct.ProviderType, acct.ProviderName)
					if txErr != nil {
						return fmt.Errorf("mysql store: failed to update provider available count: %w", txErr)
					}
				}
			}
