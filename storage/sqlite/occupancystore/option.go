...
// Option 是 occupancystore 的配置选项函数。
type Option func(*Options)

// Options 是 occupancystore 的配置参数。
type Options struct {
...
func buildClient(o *Options) (storeSqlite.Client, error) {
	builder := storeSqlite.GetClientBuilder()
	var builderOpts []storeSqlite.ClientBuilderOpt

	if o.InstanceName != "" {
		var ok bool
		builderOpts, ok = storeSqlite.GetInstance(o.InstanceName)
		if !ok {
			return nil, fmt.Errorf("occupancystore: sqlite instance %q not found", o.InstanceName)
		}
	} else if o.DSN != "" {
		builderOpts = []storeSqlite.ClientBuilderOpt{
			storeSqlite.WithClientBuilderDSN(o.DSN),
		}
	}

	if len(o.ExtraOptions) > 0 {
		builderOpts = append(builderOpts, storeSqlite.WithExtraOptions(o.ExtraOptions...))
	}
