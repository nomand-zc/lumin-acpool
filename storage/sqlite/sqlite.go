...
var ErrBreak = errors.New("sqlite scan rows break")
...
// SetClientBuilder 设置全局 SQLite client 构建器。
// 可用于注入自定义的 client 创建逻辑（如 tracing 等）。
func SetClientBuilder(builder clientBuilder) {
	globalBuilder = builder
}

// GetClientBuilder 获取当前的 SQLite client 构建器。
func GetClientBuilder() clientBuilder {
	return globalBuilder
}

// defaultClientBuilder 是默认的 SQLite client 构建器。
func defaultClientBuilder(builderOpts ...ClientBuilderOpt) (Client, error) {
	...
	db, err := sql.Open("sqlite3", o.DSN)
	if err != nil {
		return nil, fmt.Errorf("sqlite: open connection %s: %w", o.DSN, err)
	}
...
// ClientBuilderOpt 是 SQLite client 构建器的选项函数。
type ClientBuilderOpt func(*ClientBuilderOpts)

// ClientBuilderOpts 是 SQLite client 构建器的选项。
type ClientBuilderOpts struct {
	// DSN 是 SQLite 数据源名称。
	// 可以是文件路径（如 "/path/to/db.sqlite3"）、":memory:" 表示内存数据库，
	// 或带参数的路径（如 "file:test.db?cache=shared&mode=memory"）。
	DSN string
...
// WithClientBuilderDSN 设置 SQLite client 的 DSN。
...
// RegisterInstance 注册一个 SQLite 实例配置。
...
// GetInstance 根据名称获取已注册的 SQLite 实例配置。
func GetInstance(name string) ([]ClientBuilderOpt, bool) {
	if _, ok := sqliteRegistry[name]; !ok {
		return nil, false
	}
	return sqliteRegistry[name], true
}
