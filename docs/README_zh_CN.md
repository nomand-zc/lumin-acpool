...
| 子项目 | 定位 | 描述 |
|---|---|---|
| **lumin-client** | 客户端 SDK | 核心基础库，负责封装与各 AI 供应商平台的接口，提供统一的请求/响应格式转换、凭证管理、用量规则解析 |
...
| **lumin-actool** | 账号生产工具 | 各渠道供应商账号生产 CLI 工具，负责生成和导入各渠道供应商账号 |
...
- **lumin-acpool** 依赖 lumin-client，利用其 `Provider` 进行健康检查、凭证校验、用量规则获取，并在此基础上提供资源池调度能力。
- **lumin-proxy** 依赖 lumin-acpool 和 lumin-client，负责编排业务层请求，通过账号选取和模型调用完成代理转发。
- **lumin-admin** 依赖 lumin-proxy 和 lumin-acpool，为整个系统提供 Web 可视化管理界面。
- **lumin-desktop** 依赖 lumin-acpool 和 lumin-client，实现独立的本地桌面级 AI 代理应用。
- **lumin-actool** 依赖 lumin-client，用于多渠道 AI 供应商账号的批量生成与导入。
