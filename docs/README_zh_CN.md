...
| **lumin-proxy** | 代理网关 | 业务层代理网关，负责 API Key 管理、鉴权、计费和请求转发；**与 lumin-admin 配套使用** |
...
graph TB
    ...
    subgraph "网关层（lumin-proxy + lumin-admin，配套使用）"
        PROXY[lumin-proxy<br/>代理网关<br/>API Key / 鉴权 / 计费]
...
- **lumin-proxy** 和 **lumin-admin** 是 **配套使用** 的一对组合：lumin-proxy 作为面向用户的代理网关，负责处理模型请求（API Key 管理、鉴权、计费、请求转发）；lumin-admin 作为运维管理后台，负责可视化管理和系统配置。lumin-proxy 依赖 lumin-acpool 和 lumin-client；lumin-admin 依赖 lumin-acpool。
